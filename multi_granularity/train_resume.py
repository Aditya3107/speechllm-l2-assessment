...
def main():
    ...
    parser.add_argument("--resume_from", type=str, required=True, help="Path to checkpoint (adapter) to resume from")
    args = parser.parse_args()

    os.makedirs(args.out_dir, exist_ok=True)
    # CUDA_VISIBLE_DEVICES=4,5,6,7 means we have 4 gpus visible.
    # GPU 4 is index 0.
    device = torch.device("cuda:0") 
    
    log_file = os.path.join(args.out_dir, "training_log.csv")
    
    print(f"[INIT] Resuming Training from {args.resume_from} on GPU 4...")
    ...
    # Load Adapters
    print(f"[INIT] Loading LoRA adapters from {args.resume_from}")
    ...
    for epoch in range(args.start_epoch, args.start_epoch + args.num_epochs):
        ...
        print(f"[EPOCH {epoch}] loss={avg_l:.4f}  margin={avg_m:.4f}")

        with open(log_file, "a") as f:
            csv.writer(f).writerow([epoch, f"{avg_l:.4f}", f"{avg_m:.4f}"])
