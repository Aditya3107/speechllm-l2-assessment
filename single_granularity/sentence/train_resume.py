...
"""
Continued Bounded-DPO (BDPO) Training Script for Qwen2-Audio.
Usage:
  python train_resume.py --train_csv train.csv --out_dir output_cont --resume_checkpoint output/epoch_4
"""
...
def main():
    ...
    parser.add_argument("--resume_checkpoint", type=str, default=None, help="Path to checkpoint folder (e.g. out_dir/epoch_4)")
    ...
    # Initialize Log File
    log_file = os.path.join(args.out_dir, "training_log_continued.csv")
    ...
    # 2. Load or Initialize LoRA
    if args.resume_checkpoint:
        print(f"\n[RESUME] Loading adapter from {args.resume_checkpoint}...")
        model = PeftModel.from_pretrained(model, args.resume_checkpoint, is_trainable=True)
    else:
        print("\n[INIT] Creating new LoRA adapter...")
    ...
    if args.resume_checkpoint:
        try:
            start_epoch = int(args.resume_checkpoint.rstrip("/").split("_")[-1])
        except:
            start_epoch = 0
    
    print(f"Starting training from epoch {start_epoch+1} to {start_epoch + args.num_epochs}...")

    for i in range(args.num_epochs):
        current_epoch = start_epoch + i + 1
        print(f"\n[EPOCH] {current_epoch} (Batch {i+1}/{args.num_epochs})")
        
        pbar = tqdm(loader, desc=f"Ep {current_epoch}")
        epoch_loss = 0.0
        epoch_margin = 0.0
        steps = 0
        ...
        # Save Checkpoint
        ckpt_dir = os.path.join(args.out_dir, f"epoch_{current_epoch}")
        model.save_pretrained(ckpt_dir)
        processor.save_pretrained(ckpt_dir)
        
        avg_loss = epoch_loss / steps if steps > 0 else 0
        avg_margin = epoch_margin / steps if steps > 0 else 0
        print(f"[EPOCH {current_epoch}] loss={avg_loss:.4f}  margin={avg_margin:.4f}")

        with open(log_file, "a") as f:
            writer = csv.writer(f)
            writer.writerow([current_epoch, f"{avg_loss:.4f}", f"{avg_margin:.4f}"])
