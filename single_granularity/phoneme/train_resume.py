...
# --- DATASET (MODIFIED FOR MISSING PHONEMES) ---
class ComprehensiveDataset(Dataset):
    def __init__(self, path, is_train=True):
        df = pd.read_csv(path)
        self.rows = []
        
        # Check if 'target_phonemes' column exists
        has_phonemes = "target_phonemes" in df.columns

        for _, r in df.iterrows():
            if is_train:
                if "chosen" not in r or "rejected" not in r: continue
                
                # Handle missing phonemes safely
                t_phonemes = str(r["target_phonemes"]) if has_phonemes else "N/A"
                
                self.rows.append({
                    "audio_path": r["audio_path"],
                    "transcript": r["transcript"],
                    "target_phonemes": t_phonemes, 
                    "chosen": str(r["chosen"]),
...
class ComprehensiveCollator:
    ...
    def __call__(self, batch):
        ...
        for item in batch:
            ...
            # Construct Prompt (Handle N/A phonemes if necessary inside build_comprehensive_prompt too)
            # Ensure utils.py handles "N/A" gracefully or just passes it through
...
def main():
    ...
    parser.add_argument("--disable_eval", action="store_true", help="Skip automatic evaluation")
    ...
    # Load Base Model
    print(f"[INIT] Loading Qwen2-Audio base model...")
    ...
    model = Qwen2AudioForConditionalGeneration.from_pretrained(
        model_id, quantization_config=bnb_config, device_map={"": device}, torch_dtype=torch.bfloat16
    )
    model.config.use_cache = False
    model = prepare_model_for_kbit_training(model)
    
    # Resume Adapters
    print(f"[INIT] Resuming LoRA adapters from {args.resume_from}...")
    ...
    for epoch in range(args.start_epoch, args.start_epoch + args.num_epochs):
        ...
        for step, batch in enumerate(pbar):
            ...
            b_c = to_d(batch["chosen"])
            b_r = to_d(batch["rejected"])
            ...
            with torch.cuda.amp.autocast(dtype=torch.bfloat16):
                ...
                p_lp_c, p_lp_r = get_batch_logps(logits_pc, l_c), get_batch_logps(logits_pr, l_r)
                r_lp_c, r_lp_r = get_batch_logps(logits_rc, l_c), get_batch_logps(logits_rr, l_r)
        ...
        print(f"\n[SYSTEM] Saving checkpoint to {ckpt_dir}...")
        ...
        with open(log_file, "a") as f:
            csv.writer(f).writerow([f"{epoch} (TRAIN)", f"{avg_l:.4f}", f"{avg_m:.4f}"])
